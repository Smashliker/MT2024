...
class Env(gym.Env):
    ...
    def step(self, action):
        ...
        logging.debug(
            "SFC, VNF indices set to: {}, {}".format(self.sfc_idx, self.vnf_idx)
        )
        ...
        info.update(
            {res + "_utilization": val for res, val in resource_utilization.items()}
        )
        info.update({res + "_cost": val for res, val in resource_costs.items()})
    ...
    def render(self, mode="human"):
        ...
        resources = [
            [
                num,
                *[
                    str(avail_res) + " out of " + str(max_res)
                    for avail_res, max_res in zip(res[0].values(), res[1].values())
                ],
            ]
            for num, res in enumerate(
                zip(
                    self.vnf_backtrack.calculate_resources(),
                    self.vnf_backtrack.calculate_resources(False),
                )
            )
        ]
        ...
        sfcs = [
            [
                res.arrival_time,
                res.ttl,
                res.bandwidth_demand,
                res.max_response_latency,
        ...
        rep = str(
            tabulate(
                resources,
                headers=["Node", "Cpu", "Memory", "Bandwidth"],
                tablefmt="presto",
            )
        )
        rep += "\n \n \n"
        ...
        rep += (
            "Current request batch for (network) timestep "
            + str(self.network.timestep)
            + ": \n"
            + str(req_batch)
        )
        rep += "\n \n \n"
    ...
    def compute_state(self, done=False):
        ...
        network_resources = self.vnf_backtrack.calculate_resources(remaining=True)
        network_resources = np.asarray(
            [list(node_res.values()) for node_res in network_resources],
            dtype=np.float16,
        )
        ...
        max_resources = np.asarray(
            [list(node_res.values()) for node_res in max_resources], dtype=np.float16
        )
